...
ReentrantLock 是一个基于 AQS 框架的可重入锁，可以完全替代 `synchronized`，功能更加强大，更灵活，性能更好
...
- ReentrantLock 可实现选择性的通知（基于 Condition 接口），而 `synchronized` 只支持随机唤醒（`notify`）或唤醒全部（`notifyAll`）
