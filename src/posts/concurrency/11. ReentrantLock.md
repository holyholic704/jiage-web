...
ReentrantLock 是一个基于 AQS 框架的可重入锁，可以完全替代 `synchronized`，功能更加强大，更灵活，性能也更好

|  | ReentrantLock | `synchronized` |
| :- | :-: | :-: |
| 性能 | 更好 | 略差 |
| 响应中断 | 可以响应 | 只能等待其执行完成 |
| 超时时间 | 获取锁时可以添加等待时间，超过时间则停止获取 | 获取不到锁则会一直等待 |
| 支持公平锁 | 支持公平锁和非公平锁 | 只支持非公平锁 |
| 遇到异常是否可以自动释放锁 | 无法自动释放 | 会自动释放 |
| 唤醒 | 可实现选择性的通知（基于 Condition 接口） | 只支持随机唤醒（`notify`）或唤醒全部（`notifyAll`） |
...
也叫做递归锁，主要是为了 **避免产生死锁**，同一线程多次获取同一个锁不会被阻塞，而其他线程尝试获取的话会被阻塞
...
- 建议将解锁的方法放入 `finally` 块中执行，以避免发生异常时无法解锁
...
public interface Lock {
    ...
    // 尝试获取锁，成功返回 true，失败返回 false，直接返回结果，不会等待
    boolean tryLock();
...
- 公平锁：线程按照申请的顺序依次获取锁
  - 优点：按序平均分配锁资源，无论等待时间长短，线程最后总是能获取到锁，不会出现线程饿死的情况
  - 缺点：性能较差，需要依次唤醒等待队列中的线程
- 非公平锁：线程获取锁的顺序不是按照先来后到顺序的，后申请的线程可能会先获取到锁
  - 优点：整体的吞吐效率高，因为线程有几率不阻塞直接获得锁，减少了线程休眠和唤醒的开销，不必去唤醒所有线程
  - 缺点：可能会出现线程饿死的情况

#### ReentrantLock 中的实现

- 公平锁：如果当前等待队列为空，先通过 CAS 操作尝试获取锁，获取不到才会将线程放入等待队列
- 非公平锁：先通过 CAS 操作尝试获取锁，获取不到会将线程放入等待队列
...
static final class FairSync extends Sync {
    ...
    // 与 Sync 类中的 nonfairTryAcquire 方法类似，只多了检查是否有同步队列的操作
    protected final boolean tryAcquire(int acquires) {
        final Thread current = Thread.currentThread();
        int c = getState();
        // 判断当前是否有线程持有该锁
        if (c == 0) {
            // 如果当前没有同步队列，或者有同步队列且当前线程就是下一个要获取锁的线程（不是头结点，而是头结点的后继节点），且能通过 CAS 操作成功获取到锁，如果是就不需要进队列了
            // 简单来说，如果有同步队列，且同步队列中的节点目前都没有持有锁，会进行争抢
...
public final boolean hasQueuedPredecessors() {
    ...
    Node s;
    // 如果 h != t 则说明队列不为空
    // 如果 (s = h.next) == null 则说明队列中只有一个头结点
    // 如果 s.thread != Thread.currentThread() 则说明节点的后继节点持有的线程不是当前线程，因为头结点是不持有线程的
    return h != t &&
...
`acquire` 先调用 `tryAcquire` 去尝试获取锁，`tryAcquire` 方法在 FairSync 与 NonfairSync 类中被具体实现
...
private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
    ...
    if (ws > 0) {
        ...
        // 前驱节点的状态 waitStatus 是 0 或者 PROPAGATE，则将状态改为 SIGNAL
