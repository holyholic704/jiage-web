...
AQS 虽然是抽象类，但内部并没有抽象方法，取而代之的是一些只抛出异常没有具体实现的 `protected` 方法，即空方法，提供给子类去实现。这样做的好处是避免子类将所有的抽象方法都实现一遍，只需要实现自己需要的方法即可
...
`waitStatus` 是当前节点的状态，有 5 种状态，通过 CAS 操作进行转换
...
AQS 使用 `state` 表示同步状态，并且设置和获取 `state` 的方法都是被 `final` 修饰的
