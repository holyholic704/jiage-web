...
- 第一范式：表中每个字段都不可再分
- 第二范式：满足第一范式，表中所有非主键字段必须完全依赖于主键，不能只依赖主键的一部分（主要针对联合主键）
- 第三范式：满足第二范式，表中所有非主键字段都和主键直接相关，不能间接相关
  - 非主键之间应该相互独立

### 反范式

范式化可以减少数据冗余，表的体积也更小，更新操作会更快，但可能需要多个表的关联查询，较难使用索引优化

反范式基本上就是只遵守第三范式，通过存储冗余的数据来减少关联查询，也能更好的利用索引
...
也可考虑使用 BIGINT 代替 DECIMAL，使用时乘以或除以相应的倍数即可，并且同时避免浮点存储计算不精确和 DECIMAL 精确计算代价高的问题
...
- [数据库设计三大范式](https://1kuzus.github.io/23d/database-3nf/)
- [数据库的范式化和反范式化](https://www.cnblogs.com/luler/p/14958693.html)
