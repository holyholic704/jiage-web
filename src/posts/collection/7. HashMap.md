...
负载因子控制着数组存放数据的 **疏密程度**，因为需要通过 `threshold` 判断是否需要扩容，而 `threshold` 是通过 `当前容量 * 负载因子` 计算得来的
...
2 的整数幂减 1，相当于一个取反操作，例如 `10000` 减 1，也就变成了 `01111`。这时再通过与运算，就可以得到取余操作时舍弃的值了，因为 0、1 与 1 做与运算时，会保留原来的值
...
获取到 key 的 hash 值后，再通过取余操作，就可以得到该 key 对应的数组下标位置了
...
- 平方探测：从冲突的位置开始，以平方的步数开始查找，即 1^2、2^2、3^2...，直到找到空位
  ...
  - 由于是平方的增长，平方探测可能不会探测整个哈希表，这意味着即使哈希表中有空桶，平方探测也可能无法访问到它
...
final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
    ...
    if ((p = tab[i = (n - 1) & hash]) == null)
        ...
        // 如果节点 e 不为空，表示该节点 hash 与 key 与要插入的节点相同
        if (e != null) { // existing mapping for key
            V oldValue = e.value;
            // 如果允许修改已有的值，或着旧值为空，则进行赋值
            if (!onlyIfAbsent || oldValue == null)
                e.value = value;
            // LinkedHashMap 的相关操作，目前无需关心
            afterNodeAccess(e);
            // 返回旧值
            return oldValue;
        }
...
final Node<K,V> removeNode(int hash, Object key, Object value,
    ...
    if ((tab = table) != null && (n = tab.length) > 0 &&
        ...
        if (node != null && (!matchValue || (v = node.value) == value ||
            ...
            // 如果头节点就是要删除的节点，将该位置替换为后继节点即可，不用关心后继节点是不是 null
            else if (node == p)
                tab[index] = node.next;
