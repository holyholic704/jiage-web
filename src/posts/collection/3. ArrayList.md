...
RandomAccess 是一个 **标志接口**，内部没有任何方法或者注解，表示支持快速随机访问，可以以常数级的时间复杂度从集合中获取元素，并且使用普通 for 循环遍历的效率要高于使用迭代器遍历
...
// 空数组，与 EMPTY_ELEMENTDATA 区别开来，实行不同的扩容策略
...
在 ArrayList 的父类 AbstractList 中有个 `modCount` 变量，用来记录该集合的结构的修改次数，只要执行了能够 **修改集合容量的操作**，`modCount` 都会自增，如执行 `add`、`remove`、`clear` 等操作 `modCount` 都会进行自增，但执行 `set`、`get` 等操作 `modCount` 就不会自增，因为集合的容量没有改变

```java
protected transient int modCount = 0;
```

ArrayList 是线程不安全的，当你使用迭代器遍历一个集合的时候，同时也有可能有另一个线程再往里添加数据。所以 ArrayList 使用了 **fail-fast** 策略，来确保使用迭代器遍历集合时，如果有其他线程在做修改能够及时的感知

- fail-fast（快速失败）：一旦检测到可能发生错误，就立马抛出异常，程序不继续往下执行
- fail-safe（安全失败）：即便系统发生故障，也不会立即停止执行，通过隐藏错误来继续执行
  - 一般通过写时复制实现

在使用迭代器的时候，会先获取该集合的 `modCount`，存入自己的 `expectedModCount` 中，每次获取元素时，都判断一下自己的 `expectedModCount` 是否等于当前的 `modCount`，如果不一致则说明集合被人修改了，则抛出 ConcurrentModificationException 异常
...
增强 for 循环也是借助迭代器进行遍历的，`forEach` 方法内部也有 `expectedModCount` 与 `modCount` 判断，所以他们都会报错，而普通的 for 循环是可以正常执行的
...
- `new ArrayList<>(0)` 指向的是 `EMPTY_ELEMENTDATA`
- `new ArrayList<>()` 指向的是 `DEFAULTCAPACITY_EMPTY_ELEMENTDATA`
...
`new ArrayList<>(0)` 适用于一些需要创建集合，但不需要立即填充数据的场景，即便填充了数据，扩容也是从 1 开始的，容量的增长较为缓慢，从上面的结论可以看出 `new ArrayList<>(0)` 增长的容量更贴近实际所需的容量，空间利用率更大，缺点是扩容的次数相对较多
...
| :- | :-: | :-: |
| 所属 | 本地方法 | 普通方法 |
| 目标数组 | 需要提前创建好，可以与源数组是同一数组 | 会自动创键新数组，所以目标数组都是新数组 |
| 复制长度 | 需要指定长度 | 会根据起止下标自动计算 |
| 复制起始位置 | 可以从目标数组的任意位置开始复制任意数量的元素 | 只能从目标数组的开始位置进行复制 |
| 特点 | 更灵活 | 更方便 |
...
private void grow(int minCapacity) {
    ...
    // oldCapacity + oldCapacity / 2，即 1.5 倍
...
注意截取的列表并不是一个 ArrayList 类型的列表，而是 SubList 类型的，SubList 是 ArrayList 的内部类
...
SubList 使用的数组也是被截取的列表的数组，也就是说对 ArrayList 或 SubList 中的元素进行修改，在两者之中都会体现出来
...
假如当前集合的容量为 10，并且已经添加了 9 个元素

| 线程 A | 线程 B |
| :-: | :-: |
| 执行 `ensureCapacityInternal(size + 1);`，获取到 `size` 为 9 |  |
|  | 执行 `ensureCapacityInternal(size + 1);`，获取到 `size` 也为 9 |
| 执行 `elementData[size++] = e;`，执行完后 `size` 变成了 10 |  |
|  | 执行 `elementData[size++] = e;`，由于 `size` 已经变为了 10，下标超过了数组的上限，抛出 ArrayIndexOutOfBoundsException |

### 添加完成后的元素数量小于期望的数量

问题出现在 `elementData[size++] = e;`，该行代码其实分为两步
...
| 线程 A | 线程 B |
| :-: | :-: |
| `size` 为 0，将 `elementData[0]` 赋为 0 |  |
|  | 目前 `size` 仍为 0，所以将 `elementData[0]` 赋为 0，出现了 **重复赋值** 的情况，某一个线程的更新丢失 |

这时还有两个 `size++` 等待执行，由于 `size++` 也不是原子操作，可能会出现两种情况

- 两个线程读取到相同的 `size`
  - 假如两个线程初始看到的 `size` 都是 0，分别进行自增并赋值，导致最终的 `size` 为 1
    - 后面的元素会从 `elementData[1]` 开始赋值
- 一个线程完成自增，另一个线程才开始读取
  - 假如一个线程的 `size++` 执行完，此时 `size` 为 1，另一个线程读取到自增后的值，再执行 `size++`，最终的 `size` 为 2
    - 后面的元素会从 `elementData[2]` 开始赋值，而 `elementData[1]` 也会被阴差阳错赋上值，但这个值的位置可能是错误的
...
2. 添加过程中，一个线程抛出了 ArrayIndexOutOfBoundsException，原因如上
...
假如当前的 `size` 为 10，线程 A 和线程 B 都触发了扩容方法

| 线程 A | 线程 B |
| :-: | :-: |
| 执行到 `Arrays.copyOf(elementData, newCapacity)` |  |
|  | 执行到 `Arrays.copyOf(elementData, newCapacity)` |
| 执行完，将 `elementData` 指向了自己创建的新数组 |  |
| 执行 `elementData[size++] = e` 添加元素，为下标为 10 的位置赋上了值 |  |
| `size` 自增为 11 |  |
|  | 执行完，将 `elementData` 指向了自己创建的新数组，这样线程 A 添加的元素就丢失了 |
|  | 执行 `elementData[size++] = e` 添加元素，此时 `size` 已变为 11，为下标为 11 的位置赋上值 |
|  | `size` 自增为 12 |

如此这样下标为 10 的位置就被跳过了，也就出现了 `null`
