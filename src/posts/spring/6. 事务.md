...
MySQL 中是没有嵌套事务或者父子事务的概念的，但是有个 `SAVEPOINT` 的概念。从字面意思也可以看出，这是一个保存点，可以暂存当前事务的修改，如有需要可以回滚到该保存点
...
# 执行 ROLLBACK 或 COMMIT 会清空当前事务下的所有保存点
```

通过 `SAVEPOINT`，Spring 事务就可以实现嵌套事务了，外部事务是一个真正的事务，而内部事务，就是一个个 `SAVEPOINT`，可以单独进行回滚，但只能随着外部事务的提交一起提交

### 事务挂起

当事务创建时，就会被绑定到一个线程上。该线程会伴随着事务整个生命周期，直到事务提交、回滚或挂起。线程和事务的关系是一对一的，一个线程只能绑定一个事务，一个事务也只能被绑定在一个线程上
...
- [15.3.4 SAVEPOINT, ROLLBACK TO SAVEPOINT, and RELEASE SAVEPOINT Statements](https://dev.mysql.com/doc/refman/8.4/en/savepoint.html)
